        ...
        for row in rows {
            ...
            let (Some(trip_id), Some(arrival), Some(departure), Some(stop_id), Some(sequence)) = (
                ...
                cols.get(header.stop_id)
                    .map(|id| StopId::from(id.to_string())),
...
OCESN117756F1187_F:TER:FR:Line::B10C45A0-C32C-4232-85F2-4BB81B810084::87723197:87713040:10:1112:20260714,09:38:00,09:39:00,StopPoint:OCETrain TER-87721332,2,,0,0,".to_string());
        ...
        let mut expected_trips = vec![
            // A -> B, 20260501
            Trip::new(
                StopId::from("StopPoint:OCETrain TER-87723197".to_string()),
                StopId::from("StopPoint:OCETrain TER-87721282".to_string()),
                1777619760, // 9h16
                1777620480, // 9h28
            ),
            // B -> C, 20260501
            Trip::new(
                StopId::from("StopPoint:OCETrain TER-87721282".to_string()),
                StopId::from("StopPoint:OCETrain TER-87721332".to_string()),
                1777620600, // 9h30
                1777621080, // 9h38
            ),
            // A -> C, 20260501
    ...
    fn test_stop_times_rows_with_malformed_sequence() {
        ...
        assert_eq!(trips[0].origin, StopId::from("STOP_A".to_string()));
        assert_eq!(trips[0].destination, StopId::from("STOP_C".to_string()));
    }
