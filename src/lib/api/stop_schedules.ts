...
export const fetchStopSchedules = async ({
	stop,
	from
}: z.infer<typeof schema>): Promise<Destination[]> => {
	...
	const res = schedules
		...
		.toSorted((a, b) => (a.stops[0].date_time > b.stops[0].date_time ? 1 : -1)) as LineSchedule;

	return extractDestinationsFromSchedules(res);
};
...
export const extractDestinationsFromSchedules = (schedules: Schedule[]): Destination[] => {
	...
	for (const schedule of schedules) {
		...
		for (const stop of schedule.stops) {
			...
			const duration = dayjs(stop.date_time).diff(dayjs(origin.date_time), 'second');

			const existing = destinations.get(stop.id);
			if (existing === undefined) {
				destinations.set(stop.id, { stop, duration, schedule });
			} else {
	...
	const res = [...destinations.values()].sort((a, b) => a.duration - b.duration);
	return res;
};
